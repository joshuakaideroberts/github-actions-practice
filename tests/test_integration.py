...
class TestIntegration:
    ...
    def test_multiple_encoding_conversions(self):
        ...
        # Use ASCII-compatible content to avoid encoding issues during conversion
        ...
        # Convert UTF-8 → ASCII
        ascii_file = self.encoder.encode_file(original_file, encoding="ascii")
        
        # Convert ASCII → Latin-1  
        latin1_file = self.encoder.encode_file(ascii_file, encoding="latin-1")
        
        # Convert Latin-1 → UTF-8
        final_file = self.encoder.encode_file(latin1_file, encoding="utf-8")
    ...
    def test_utf16_encoding_with_bom(self):
        """Test UTF-16 encoding specifically with BOM handling."""
        original_content = "Hello, World! Test UTF-16 encoding."
        
        # Create original file
        original_file = self.create_test_file(original_content, "utf16_test.txt")
        
        # Convert to UTF-16
        utf16_file = self.encoder.encode_file(original_file, encoding="utf-16")
        
        # Verify the file was created and contains correct content
        assert os.path.exists(utf16_file)
        with open(utf16_file, 'r', encoding='utf-16') as f:
            result_content = f.read()
        assert result_content == original_content
        
        # Convert back to UTF-8
        utf8_file = self.encoder.encode_file(utf16_file, encoding="utf-8")
